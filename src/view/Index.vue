<template>
  ...
  <!-- 头部 -->
    <div class="header">
    <!-- 搜索框 -->
      <router-link to="/Search"><div class="search gray">请输入线路、目的地查询...</div></router-link>
      ...
      <div class="swipe">
        <mt-swipe :auto="2000" >
          <mt-swipe-item  v-for="item in imgList">
            <a :href='item.url'><img :src="baseurl+item.img"/></a>
          </mt-swipe-item>
        </mt-swipe>
      </div>
    </div>
  ...
  <!-- 八大导航 -->
      <div class="nav">
        <div class="nav-item"  v-for="item in nav">
          <router-link :to="{ path: 'secondIndex', query: { id: item.id}}">
            <div class="icon iconfont" :class='item.bg' :id='item.id' v-html='item.icon'></div>
            <p :class="item.name=='私人定制'?'pink':'navColor'">{{item.name}}</p>
          </router-link>
        </div>
      </div>
       <div class="empty"></div>
  <!-- 内容开始 -->
    <div class="content clear" v-if="hot.length!=0">
      <!-- 热门线路 -->
      <p class="title">
      ...
      <div class="line"></div>
      <div class="box" v-for="(item, index) in hot" v-bind:class="{'right':index%2==1}">
      <router-link :to="{ path: 'detail', query: { id: item.id }}" ><img :src="baseurl+item.imgurl"></router-link>
    ...
    <div class="content" style="padding-bottom:1rem">
      ...
      <mt-navbar class="page-part" v-model="selected">
        <mt-tab-item :id='index' v-for="(item,index) in nav">{{item.name}}</mt-tab-item>
      </mt-navbar> 
      </div>
      <!-- 路线内容 -->
      <mt-tab-container v-model="selected">
        <mt-tab-container-item :id="num-1" v-for="num in 8">
          <div class="detail_box" v-for="item in detail[num-1]">
           <router-link :to="{ path: 'detail', query: { id: item.id }}" >
            <img v-lazy="baseurl+item.imgurl"  class="bg_img"/>
            <p class="name">{{item.title}}</p>
            <p class="info gray">{{item.sub_title}}</p>
            <p class="" style="margin-top:0.1rem;display:inline-block;margin-bottom:0.2rem">
              ...
              <p class="gray right" style="margin-top:0.2rem;">已购买{{item.buy_number}}人</p>
            </p>
            <p>
              <span class="tag" v-for="tags in item.keyword.split('/')">{{tags}}</span>
            </p>
            <!-- 星星评分 -->
            <div class="star-box">
              <img src="static/star.png" v-for="item in 4"/><img src="static/stars.png" v-for="item in 1">
            </div>
            </router-link>
          </div>
...
export default {
  ...
  data: function () {
    return {
      ...
      selected: '1',
      detail:[],
      nav:[{"name":"短线游","bg":"bg_1ac8f1","icon":"&#xe631;",'id':1},
          {"name":"长线游","bg":"bg_67c705","icon":"&#xec0a;",'id':2},
          {"name":"出境游","bg":"bg_1ccfc7","icon":"&#xedc3;",'id':3},
          {"name":"门票","bg":"bg_2fa1f1","icon":"&#xe603;","id":7},
          {"name":"室内·轰趴","bg":"bg_ff6162","icon":"&#xe65d;",'id':0},
          {"name":"户外·拓展","bg":"bg_01c46c","icon":"&#xe69c;",'id':4},
          {"name":"城会玩","bg":"bg_68c70a","icon":"&#xe832;",'id':5},
          {"name":"私人定制","bg":"bg_ff6160","icon":"&#xe600;",'id':6}],
      hide:"hide",
      addressChoose:"",
      addressSlots: [
          {
    ...
    var geolocation = new qq.maps.Geolocation("TKPBZ-XLA3U-JSRVJ-4CMDD-R6BF6-I5FV4", "myapp");
      geolocation.getLocation(function(response){
        if(!response.district){
          self.address = "全国"
          localStorage.address = self.address;
        }else{
          var address="";
        ...
        //请求数据
        http(api.Index.method, api.Index.url,{local:self.address},function(data){
          console.log(data)
          self.imgList=data.banner;
          self.hot=data.hot;
          self.selected=0;
          self.detail=data.tab;
  ...
  methods: {
    ...
    choose:function(){
      ...
      this.hide="hide";
      http(api.Index.method, api.Index.url,{local:self.addressChoose},function(data){
          self.imgList=data.banner;
          self.hot=data.hot;
          self.selected=0;
          self.detail=data.tab;
